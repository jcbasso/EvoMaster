...
func buildInstrumentationTool(t *testing.T) (path string) {
	toolDir, err := ioutil.TempDir("", "test-evomaster-instrumentation")
	require.NoError(t, err)
	toolPath := filepath.Join(toolDir, "evomaster")
	if runtime.GOOS == "windows" {
		toolPath += ".exe"
	}
	cmd := exec.Command(godriver, "build", "-o", toolPath, "github.com/sqreen/go-agent/sdk/client-go")
